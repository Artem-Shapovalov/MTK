...
mtk::display& mtk::image::init()
{
  return *this;
}

mtk::display& mtk::image::deinit()
{
  return *this;
}
...
mtk::display& mtk::part::init()
{
  return *this;
}

mtk::display& mtk::part::deinit()
{
  return *this;
}
...
mtk::draw::draw(mtk::display& d) : d(d)
{
  d.init();
}
...
mtk::draw& mtk::draw::frame(int x1, int y1, int x2, int y2, bool color)
{
  for (int x = x1; x <= x2; x++)
  {
    d.set(x, y1, color).set(x, y2, color);
  }

  for (int y = y1; y <= y2; y++)
  {
    d.set(x1, y, color).set(x2, y, color);
  }
...
mtk::draw& mtk::draw::print(const char* str, int x, int y, bool invert)
{
  ...
  while (*str)
  {
    ...
    x += char_tab[*(unsigned char*)str]->get_info().w;
    str++;
  }
...
mtk::draw& mtk::draw::refresh()
{
  d.refresh();
  return *this;
}

mtk::draw& mtk::draw::clear()
{
  d.clear();
  return *this;
}
...
mtk::widget* mtk::core::find_nearest(mtk::widget* tw, mtk::core::direction dir)
{
  ...
  widget* iter = s->w;
  int tx = (tw->x1 + tw->x2) / 2;
  int ty = (tw->y1 + tw->y2) / 2;
  ...
  while (iter)
  {
    ...
    int cx = (iter->x1 + iter->x2) / 2;
    int cy = (iter->y1 + iter->y2) / 2;

    if (dir == direction::up    && cy >= ty)
    {
      iter = iter->next;
      continue;
    }

    if (dir == direction::down  && cy <= ty)
    {
      iter = iter->next;
      continue;
    }

    if (dir == direction::left  && cx >= tx)
    {
      iter = iter->next;
      continue;
    }

    if (dir == direction::right && cx <= tx)
    {
      iter = iter->next;
      continue;
    }
